...
function SmartAI:useCardDuel(duel, use)
	...
	for _, enemy in ipairs(enemies) do
		...
		useduel = n1 >= n2 or self:needToLoseHp(self.player)
					...
					or ((self.player:hasSkill("jianxiong") or self.player:getMark("shuangxiong") > 0) and sgs.isGoodHp(self.player))
...
function SmartAI:useCardSnatchOrDismantlement(card, use)
	...
	for _, enemy in ipairs(enemies) do
		if enemy:hasArmorEffect("eight_diagram") and not self:needToThrowArmor(enemy) and (not isDiscard or self.player:canDiscard(enemy, enemy:getArmor():getEffectiveId())) then
...
function SmartAI:useCardIndulgence(card, use)
	...
	local getvalue = function(enemy)
		...
		if enemy:hasSkills("nostuxi|noszhenlie|guanxing|qinyin|zongshi|tiandu") then value = value - 3 end
