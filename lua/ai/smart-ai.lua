...
function SmartAI:filterEvent(event, player, data)
	...
	elseif event == sgs.PreCardUsed then
		...
		if callback then
			...
			if not sgs.isRolePredictable() and card:isKindOf("SkillCard") and not card:targetFixed() then
...
function SmartAI:askForCardChosen(who, flags, reason)
	...
	if self:isFriend(who) then
		...
		if flags:match("e") then
			if who:hasWeapon("crossbow") and getCardsNum("Slash", who) > 1 then
				for _, friend in ipairs(self.friends) do
		...
		if flags:match("e") then
			if who:getArmor() and self:evaluateArmor(who:getArmor(), who) > 0
				and not self:needToThrowArmor(who) then
...
function SmartAI:getAoeValueTo(card, to, from)
	...
	if self:aoeIsEffective(card, to, from) then
		...
		if self.room:getMode() ~= "06_3v3" and self.room:getMode() ~= "06_XMode" and not from:hasSkill("jueqing") then
			if to:getHp() == 1 and not to:hasSkill("buqu") and from:isLord() and sgs.evaluateRoleTrends(to) == "loyalist" and self:getCardsNum("Peach") == 0 then
...
function SmartAI:doNotDiscard(to, flags, conservative)
	...
	if flags == "h" or (flags == "he" and not to:hasEquip()) then
		...
		if to:hasSkill("tuntian") and to:getPhase() == sgs.Player_NotActive and (not conservative or #self.enemies > 1) then return true end
