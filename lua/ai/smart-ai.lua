...
version = "QSanguosha AI 20130401 (V0.96875 Alpha)"
...
sgs.ai_card_intention.general = function(from, to, level)
	...
	if sgs.evaluatePlayerRole(to) == "loyalist" then
		if ((sgs.role_evaluation[from:objectName()]["loyalist"] > 100 and level > 0 and sgs.current_mode_players["rebel"] > 0)
			or (sgs.role_evaluation[from:objectName()]["loyalist"] < -100 and level < 0)) and sgs.current_mode_players["renegade"] > 0 then
	...
	if sgs.evaluatePlayerRole(to) == "rebel" then
		if ((sgs.role_evaluation[from:objectName()]["loyalist"] < -100 and level > 0) or (sgs.role_evaluation[from:objectName()]["loyalist"] > 0 and level < 0))
	...
	elseif self.role == "rebel" then
		...
		if sgs.evaluatePlayerRole(player) == "neutral" then
			...
			for _, aplayer in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				...
				if sgs.ai_role[aplayer:objectName()] == "renegade" or sgs.evaluatePlayerRole(player) == "loyalist" then
					current_enemy_num = current_enemy_num + 1
				end
			end
...
function SmartAI:getOverflow(player)
	player = player or self.player
	local kingdom_num = 0
	if player:hasSkill("yongsi") and player:getPhase() <= sgs.Player_Discard and not (player:hasSkill("keji") and not player:hasFlag("KejiUseSlash")) then
			local kingdoms = {}
